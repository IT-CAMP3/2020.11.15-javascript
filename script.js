...
function generate_letters() {
	...
	for(i=0; i<35; i++) {
		tresc_diva = tresc_diva + '<div class="letter" onClick="check(' + i + ')" id="lit' + i + '">' + litery[i] + '</div>';
		if((i+1) % 7 == 0)
...
function check(nr) {
	...
	if(trafienie) {
		...
		document.getElementById(element_id).style.border = "3px solid #005500";
		...
		document.getElementById(element_id).style.border = "3px solid #550000";
		document.getElementById(element_id).setAttribute("onclick", ";")
	}
